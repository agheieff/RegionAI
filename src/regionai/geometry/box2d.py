...
class Box2D:
    ...
    def __init__(
        self, x_min: float, y_min: float, x_max: float, y_max: float, device: str = None
    ):
        ...
        self.min_corner = torch.tensor(
            [x_min, y_min], dtype=torch.float32, device=self.device
        )
        self.max_corner = torch.tensor(
            [x_max, y_max], dtype=torch.float32, device=self.device
        )

        # Validate box
        if torch.any(self.min_corner > self.max_corner):
            raise ValueError(
                f"Invalid box: min corner {self.min_corner} > max corner {self.max_corner}"
            )
    ...
    def from_center_size(
        cls, center: Tuple[float, float], size: Tuple[float, float], device: str = None
    ):
    ...
    def contains(self, other: "Box2D") -> bool:
        ...
        return (
            torch.all(self.min_corner <= other.min_corner)
            and torch.all(other.max_corner <= self.max_corner)
        ).item()
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Box2D(min={self.min_corner.tolist()}, "
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Box2D(center=[{center[0]:.2f}, {center[1]:.2f}], "
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, Box2D):
            return NotImplemented
        return torch.allclose(self.min_corner, other.min_corner) and torch.allclose(
            self.max_corner, other.max_corner
        )

...
from .math_curriculum import MathCurriculum
...
class MathCurriculumGenerator(CurriculumGenerator):
    """Generates mathematical theorem proving problems."""
    
    name = "math_foundations"
    description = "Mathematical theorem proving from Lean files"
    
    def __init__(self):
        super().__init__()
        self.math_curriculum = MathCurriculum()
        self._loaded = False
    ...
    def generate(self, config: CurriculumConfig) -> List[Problem]:
        ...
        # Check for topic filter in extra params
        if config.extra_params and 'topics' in config.extra_params:
            topics = config.extra_params['topics']
        
        problems = self.math_curriculum.get_problems(
            difficulty_range=difficulty_range,
            topics=topics,
            limit=config.num_problems
        )

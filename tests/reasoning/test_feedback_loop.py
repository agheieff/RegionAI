...
'''
    ...
    for node in hub.rkg.graph.nodes():
        if isinstance(node, Heuristic) and node.implementation_id:
            initial_scores[node.name] = node.utility_score
            print(f"  Initial score for '{node.name}': {node.utility_score:.3f}")
    ...
    # Get updated utility scores
    updated_scores = {}
    score_changes = {}
    
    for node in hub.rkg.graph.nodes():
        if isinstance(node, Heuristic) and node.implementation_id:
            updated_scores[node.name] = node.utility_score
            if node.name in initial_scores:
                change = node.utility_score - initial_scores[node.name]
                score_changes[node.name] = change
                print(f"\n  '{node.name}':")
                print(f"    Initial: {initial_scores[node.name]:.3f}")
                print(f"    Updated: {node.utility_score:.3f}")
